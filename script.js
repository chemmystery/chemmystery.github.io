 ...
 * Dynamic GitHub Issue Fetching & Automated Home Slider
 */
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function parseMarkdown(body) {
        if (!body) return { 
            image: 'https://via.placeholder.com/1280x720/e0f2fe/0f172a?text=Image+Pending', 
            text: 'Details coming soon.' 
        };
        
        const imgRegex = /!\[.*?\]\((.*?)\)/;
        const imgMatch = body.match(imgRegex);
        const imageUrl = imgMatch ? imgMatch[1] : 'https://via.placeholder.com/1280x720/e0f2fe/0ea5e9?text=No+Image+Provided';
