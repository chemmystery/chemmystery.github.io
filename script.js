 ...
 * Dynamic GitHub Issue Fetching, Bulletproof Parsing, & Automated Home Slider
 */
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- Indestructible GitHub API & Data Parsing ---
    function parseMarkdown(body) {
        const fallbackImg = 'https://placehold.co/600x400/e0f2fe/0ea5e9?text=Image+Loading...';
        
        if (!body) return { image: fallbackImg, text: '' };
        
        let imageUrl = fallbackImg;
        
        // 1. Hunt for the exact GitHub User Attachment URL (Fixes your screenshot issue)
        const gitHubUrlMatch = body.match(/(https:\/\/github\.com\/user-attachments\/assets\/[a-zA-Z0-9-]+)/);
        
        // 2. Fallback: Hunt for any standard src="..." attribute inside an img tag
        const srcMatch = body.match(/src=["'](.*?)["']/);
        
        // 3. Fallback: Hunt for classic markdown images ![alt](url)
        const mdMatch = body.match(/!\[.*?\]\((.*?)\)/);

        // Assign the extracted image
        if (gitHubUrlMatch) {
            imageUrl = gitHubUrlMatch[1];
        } else if (srcMatch) {
            imageUrl = srcMatch[1];
        } else if (mdMatch) {
            imageUrl = mdMatch[1];
        ...
        // Clean the text body to remove the image syntax and GitHub's auto-generated YAML headings
        let textContent = body
            .replace(/<img[^>]*>/g, '') // Erase HTML image tags
            .replace(/!\[.*?\]\(.*?\)/g, '') // Erase Markdown images
            .replace(/(https:\/\/github\.com\/user-attachments\/assets\/[a-zA-Z0-9-]+)/g, '') // Erase raw links
            .replace(/### .*/g, '') // Erase ALL GitHub issue headings like "### Image Upload"
            .trim()
            .replace(/\n{2,}/g, '\n') // Condense huge gaps
            .replace(/\n/g, '<br>'); // Keep normal line breaks
    ...
    async function initHomeSlider() {
        ...
        if (!issues || issues.length === 0) {
            track.innerHTML = `<div style="width:100%; text-align:center; padding: 4rem; color: var(--text-muted);">New batches announcing soon.</div>`;
            return;
        }
    ...
    async function loadDynamicContent(tabName) {
        ...
        if (!issues || issues.length === 0) {
            gridElement.innerHTML = `
                <div style="grid-column: 1/-1; text-align: center; padding: 4rem 2rem; color: var(--text-muted); background: var(--glass-bg); border-radius: 24px; border: 1px solid var(--glass-border); box-shadow: var(--glass-shadow);">
                    ...
                    <p style="font-weight: 500; font-size: 1.1rem;">Updating content shortly...</p>
                </div>`;
            return;
        }
        ...
        issues.forEach((issue, index) => {
            ...
            if (tabName === 'gallery') {
                ...
                // Do not render empty description tags if there is no text
                const textHTML = text ? `<p class="card-desc">${text}</p>` : '';
